...
export class FirestoreService {
  ...
  messageCollectin: AngularFirestoreCollection<IMessage>;
  userCollection: AngularFirestoreCollection<IUser>;

  constructor(public af: AngularFirestore) {
    this.messageCollectin = this.af.collection<IMessage>(
  ...
  messageAdd(message: IMessage) {
    return this.messageCollectin.add(message);
  }

  chatInit(): Observable<IChat[]> {
    return this.messageCollectin.valueChanges( {idField: 'messageId'} )
           .pipe(
             concatMap(
               async messages => {
